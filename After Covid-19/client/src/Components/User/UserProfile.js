...
const UserProfile = () => {
  ...
  useEffect(async () => {
    try {
      ...
      const res = await axios.get(`/patients/${state[0].id}`);
      // console.log("patient id", state, res);
      setpatient(res.data.result[0]);
      // console.log("dd", res.data.result[0]);

      const res2 = await axios.post(`/doctors/getappointementpatient`, {
  ...
  const deleteBooking = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("/doctors/deletebooking", {
