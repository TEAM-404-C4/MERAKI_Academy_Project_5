...
const getDoctorAppointmentByDoctorId=(req,res)=>{
  ...
  connection.query(query, data, (err, result) => {
    ...
    res.status(200).json({success: true,message:`Retrieve All Appointment for Doctor =>${data[0]}`});
  });};
...
const getDoctorAppointmentByPatientId=(req,res)=>{
  ...
  connection.query(query, data, (err, result) => {
    ...
    res.status(200).json({success: true,message:`Retrieve All Appointment for Patient =>${data[0]}`});
  });
};
